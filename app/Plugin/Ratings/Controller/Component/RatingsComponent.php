	...
	public function initialize(&$controller, $settings = array()) {
		$this->controller = $controller;
 		if ($this->enabled == true) {
			...
			$controller->helpers[] = 'Users.Rating';
		}
	}
	...
	public function rate($rate, $rating, $user, $redirect = false) {
		...
		if ($controller->{$this->modelName}->exists(true)) {
			if ($controller->{$this->modelName}->saveRating($rate, $user, $rating)) {
				...
				$message = __d('platform', 'Your rate was successfull.', true);
				$status = 'success';
			} else {
				$message = __d('platform', 'You have already rated.', true);
				$status = 'error';
			}
		} else {
			$message = __d('platform', 'Invalid rate.', true);
			$status = 'error';
		}
