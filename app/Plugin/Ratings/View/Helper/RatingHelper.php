 ...
 */
	...
	public function display($options = array(), $urlHtmlAttributes = array()) {
		...
		if (empty($options['item'])) {
			throw new Exception(__d('platform', 'You must set the id of the item you want to rate.', true), E_USER_NOTICE);
		}
 ...
 */
	public function starForm($options = array(), $urlHtmlAttributes = array()) {
		$options = array_merge($this->defaults, $options);
		if (empty($options['item'])) {
			trigger_error(__d('platform', 'You must set the id of the item you want to rate.', true), E_USER_NOTICE);
		}
		$result = '';
		...
		if ($options['createForm']) {
			$result .= $this->Form->submit(__d('platform', 'Rate!', true)) . "\n";
